...
from app.services.transcription_service import save_transcription, transcribe_video_with_timestamps, whisper_available
from app.services.video_artifact_service import (
    ...
    save_transcription as save_transcription_artifact,
)
from app.validators.video_validators import parse_clip_selection, parse_runtime_settings, validate_filename
...
def upload_video():
    ...
    settings = load_frame_ai_settings()
    try:
        ai_config = _resolve_ai_config(request.form.get("ai_task") or request.form.get("task_type"), request.form.get("model_path"))
        clip_config = parse_clip_selection(request.form)
        runtime_config = parse_runtime_settings(request.form, defaults={"frame_stride": settings.frame_stride, "max_frames": settings.max_frames, "confidence": settings.confidence})
    except ValueError as exc:
        return jsonify({"error": str(exc)}), 400
    ...
    save_ai_config(new_video.id, task_type=ai_config["task_type"], task_label=ai_config["task_label"], model_path=ai_config["model_path"], model_name=ai_config["model_name"], frame_stride=runtime_config.frame_stride, confidence_threshold=runtime_config.confidence_threshold, max_frames=runtime_config.max_frames, clip_start_sec=clip_config.clip_start_sec, clip_end_sec=clip_config.clip_end_sec)
    save_processing_state(new_video.id, progress=1, stage="queued", eta_seconds=None, message="Upload concluído. Aguardando inicio do processamento.")
    try:
        job_queue = get_job_queue()
        job_id = job_queue.enqueue(new_video.id)
        update_job_id(new_video, job_id)
        current_app.logger.info("upload_video:job_enqueued video_id=%s job_id=%s", new_video.id, job_id)
    except Exception as exc:
        current_app.logger.error("upload_video:enqueue_failed video_id=%s error=%s", new_video.id, exc)
        return jsonify({"error": "Falha ao enfileirar processamento"}), 500
    return jsonify({"message": "Upload realizado com sucesso", "video": _serialize_video(new_video), "next_steps": {"video_saved_in": to_repo_relative(filepath), "analysis_will_be_saved_in": to_repo_relative(analysis_file_path(new_video.id)), "annotated_will_be_saved_in": to_repo_relative(annotated_video_path(new_video.id)), "transcription_will_be_saved_in": "uploads/transcriptions/video_{new_video.id}.json", "analysis_status": "PROCESSANDO_IA", "clip_selection": {"clip_start_sec": clip_config.clip_start_sec, "clip_end_sec": clip_config.clip_end_sec}, "runtime_settings": {"frame_stride": runtime_config.frame_stride, "max_frames": runtime_config.max_frames, "confidence_threshold": runtime_config.confidence_threshold}}})
...
def get_video_analysis(video_id: int):
    ...
    return jsonify({"video_id": video.id, "filename": video.filename, "status": video.status, "available": True, "message": "Análise carregada com sucesso.", "analysis": analysis, "available_variants": variants, "selected_variant_id": analysis.get("analysis_variant_id"), "ai_config": ai_config, "storage": storage})
...
def delete_video_analysis_by_id(video_id: int):
    ...
    if requested_variant:
        ...
        return jsonify({"message": "Versão selecionada removida com sucesso", "video": _serialize_video(video), "available_variants": list_analysis_variants(video.id)})
...
def generate_video_transcription_by_id(video_id: int):
    ...
    return jsonify({"message": "Transcrição automática gerada com sucesso", "video": _serialize_video(video), "transcription": saved})
...
def get_video_file(video_id: int):
    ...
    return send_file(filepath, mimetype=mimetype or "application/octet-stream", as_attachment=False, download_name=video.filename)
...
def get_annotated_video_file(video_id: int):
    ...
    return send_file(filepath, mimetype="video/mp4", as_attachment=False, download_name=filepath.name)
...
def update_video_ai_config(video_id: int):
    ...
    try:
        ...
        return jsonify({"error": str(exc)}), 400
    saved = save_ai_config(video.id, task_type=ai_config["task_type"], task_label=ai_config["task_label"], model_path=ai_config["model_path"], model_name=ai_config["model_name"], frame_stride=runtime_config.frame_stride, confidence_threshold=runtime_config.confidence_threshold, max_frames=runtime_config.max_frames, clip_start_sec=clip_config.clip_start_sec, clip_end_sec=clip_config.clip_end_sec)
...
def reprocess_video_by_id(video_id: int):
    ...
    if payload:
        try:
            ...
            return jsonify({"error": str(exc)}), 400
        save_ai_config(video.id, task_type=ai_config["task_type"], task_label=ai_config["task_label"], model_path=ai_config["model_path"], model_name=ai_config["model_name"], frame_stride=runtime_config.frame_stride, confidence_threshold=runtime_config.confidence_threshold, max_frames=runtime_config.max_frames, clip_start_sec=clip_config.clip_start_sec, clip_end_sec=clip_config.clip_end_sec)
    try:
        job_queue = get_job_queue()
        job_id = job_queue.enqueue(video.id)
        update_job_id(video, job_id)
        current_app.logger.info("reprocess:job_enqueued video_id=%s job_id=%s", video.id, job_id)
    except Exception as exc:
        current_app.logger.error("reprocess:enqueue_failed video_id=%s error=%s", video.id, exc)
...
def cancel_video_processing_by_id(video_id: int):
    ...
    save_processing_state(video.id, progress=processing.get("processing_progress", 0), stage="cancelled", eta_seconds=None, message="Cancelamento solicitado. O vídeo e análises concluídas serão mantidos.")
...
def list_ai_models():
    ...
    return jsonify({"models": models, "tasks": [{"task_type": task_type, "task_label": task_label} for task_type, task_label in tasks]})


def get_system_version():
    return jsonify({**build_version_payload(), "backend_name": "DeMoviefy Backend"})
    ...
    with flask_app.app_context():
        ...
        try:
            ...
            def report_analysis_progress(progress_payload: dict) -> None:
                ...
                save_processing_state(video_id, progress=progress_value, stage="analyzing", eta_seconds=eta_seconds, message=f"Analisando frames {sampled_frames}/{estimated_total}.")

            summary = analyze_video_frames(video_path=str(video_path), model_path=ai_config["model_path"], task_type=ai_config["task_type"], frame_stride=int(ai_config.get("frame_stride") or flask_app.config.get("FRAME_AI_FRAME_STRIDE", 8)), conf_threshold=float(ai_config.get("confidence_threshold") or flask_app.config.get("FRAME_AI_CONFIDENCE", 0.35)), max_frames=int(ai_config.get("max_frames") or flask_app.config.get("FRAME_AI_MAX_FRAMES", 300)), clip_start_sec=float(ai_config.get("clip_start_sec") or 0.0), clip_end_sec=ai_config.get("clip_end_sec"), annotated_output_path=str(annotated_path), progress_callback=report_analysis_progress, cancellation_requested=cancellation_requested, logger=flask_app.logger)
            summary["selected_model"] = ai_config
            analysis_path = save_analysis(video_id, summary)
            save_processing_state(video_id, progress=100, stage="completed", eta_seconds=0, message="Processamento concluído. Clique em 'Gerar transcrição IA' para transcrição sob demanda.")
            update_status(video, "PROCESSADO")
            flask_app.logger.info("process_video:completed video_id=%s filename=%s analysis=%s", video.id, video.filename, analysis_path)
        except ProcessingCancelled:
            db.session.rollback()
            video = get_video(video_id)
            if video:
                update_status(video, "CANCELADO")
            save_processing_state(video_id, progress=0, stage="cancelled", eta_seconds=None, message="Processamento cancelado. O vídeo e análises concluídas foram mantidos.")
            annotated_temp_path = annotated_video_temp_path(video_id)
            if annotated_temp_path.exists():
                annotated_temp_path.unlink()
            ...
            save_processing_state(video_id, progress=100, stage="error", eta_seconds=None, message="Falha durante o processamento.")
            annotated_temp_path = annotated_video_temp_path(video_id)
            if annotated_temp_path.exists():
                annotated_temp_path.unlink()
